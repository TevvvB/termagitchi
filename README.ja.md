...
| Codex CLI | フックは動くが、先に Codex 側で信頼する必要がある（下記参照）。ステータスラインはないので、ペット表示は下のシェル用スニペットで |
...
**Codex のフックは実行前に信頼が必要です。** ここが肝心で、誰も教えてくれません。Codex は新しい／変更されたフックを受け入れるまで実行せず、未信頼のフックは完全に無言で、`codex doctor` もフックについて何も言いません。なので `pets install --harness=codex` は一見うまくいったように見えて、次のセッション開始で「はい」と言うまで何もしません。Codex 0.149.0 で検証済み — 同じ設定は `--dangerously-bypass-hook-trust` 付きでのみ発火します。

信頼したあとは動きます。Codex のペイロードには `session_id` があり、それがクリーチャーのキーなので、エージェントは登録され、`pets den` と `pets party` は Claude Code のときと同じように埋まります。Codex ペイロードには `workspace` オブジェクトがないので、巣の名前は git から取ります。
